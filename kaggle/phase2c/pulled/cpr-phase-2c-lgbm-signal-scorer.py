"""
cpr_phase2c_kernel.py â€” Phase 2c: Regime-Conditional LightGBM HPO Signal Scorer
...
Outputs to /kaggle/working/:
  lgbm2c_global.txt        â€” global model (fallback)
  lgbm2c_regime_{0..3}.txt â€” per-regime models (states 0-3)
  shap_weights2c.json      â€” merged SHAP feature importances
  phase2c_metrics.json     â€” CV AUC, val AUC, per-regime stats
"""
...
# â”€â”€ Feature sets â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
INTERACTION_FEATURES = [
    'cpr_vol_interaction',      # cpr_compress Ã— vol_rank
    'regime_momentum',          # hmm_regime Ã— mom5
    'cpr_rsi_squeeze',          # (1 - cpr_width_pct) Ã— rsi14
    'overlap_vol_signal',       # cpr_overlap_pct Ã— cpr_zone_vol_ratio
    'rs_direction_alignment',   # (market_rs_5d + sector_rs_5d) Ã— direction
]  # 5 interaction features

FEATURE_COLS = BASE_FEATURES + INTERACTION_FEATURES  # 46 total
...
# â”€â”€ Optuna HPO â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€

def hpo_objective(trial, X_tr, y_tr, n_splits=CV_SPLITS):
    params = {
        'objective':        'binary',
        'metric':           'auc',
        'verbosity':        -1,
...
def run_hpo(X_tr, y_tr, n_trials=N_TRIALS, label='global'):
    print(f"  Optuna HPO ({n_trials} trials, {CV_SPLITS}-fold TS-CV) [{label}]...")
    study = optuna.create_study(direction='maximize',
                                 sampler=optuna.samplers.TPESampler(seed=RANDOM_SEED))
    study.optimize(lambda t: hpo_objective(t, X_tr, y_tr),
...
def get_shap_weights(model, X_sample):
    try:
        ...
        print(f"    SHAP failed: {e} â€” using split importance.")
...
def main():
    ...
    # â”€â”€ 1. Global model â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ 2. Per-regime models â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    print("\n[2/2] Per-regime models...")
    for regime in range(N_REGIMES):
        mask_tr = df_tr['hmm_regime'] == regime
        mask_va = df_va['hmm_regime'] == regime
        mask_te = df_te['hmm_regime'] == regime

        n_regime = mask_tr.sum()
        print(f"\n  Regime {regime}: {n_regime:,} train samples")

        if n_regime < MIN_REGIME_N:
            print(f"  Skip (< {MIN_REGIME_N}) â€” global model used for this regime.")
            metrics['regimes'][regime] = {'status': 'skipped', 'n_train': int(n_regime)}
            continue

        Xr_tr = df_tr.loc[mask_tr, FEATURE_COLS]
        yr_tr = df_tr.loc[mask_tr, target]
        Xr_va = df_va.loc[mask_va, FEATURE_COLS]
        yr_va = df_va.loc[mask_va, target]
        Xr_te = df_te.loc[mask_te, FEATURE_COLS]
        yr_te = df_te.loc[mask_te, target]
        ...
        # Use 50 trials per regime (budget split)
        bp_r, cv_r = run_hpo(Xr_hpo, yr_hpo, n_trials=50, label=f'regime_{regime}')
    ...
    # â”€â”€ Merge SHAP weights (global + regime blend) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
    ...
    # â”€â”€ Save metrics â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
